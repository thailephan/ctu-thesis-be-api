...
module.exports = {
    ...
    async getAll(queryParams: IQueryParams = {}) {
        ...
        let sql = `select id, name, updated_by, created_by, status,
            ceil(extract(epoch from created_at)) as created_at,
            ceil(extract(epoch from updated_at)) as updated_at from courses where status >= 1`
        const params: any[] = [];
    ...
    async create({name, created_by = null, updated_by = null, status = 1}: {name: string, created_by: number | null, updated_by: number | null, status: number}) {
        const now = new Date();
        const query = `insert into courses(name, created_by, updated_by, updated_at, created_at, status) VALUES ($1, $2, $3, $4, $5, $6) returning *`;
