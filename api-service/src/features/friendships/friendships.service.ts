...
module.exports = {
    async count(condition: ICondition) {
        ...
        try {
            let sql = `select count(*) from friendship`;
            if (search) {
    ...
    async create({created_by, id_receiver, id_sender, status = 1, updated_by}: {id_sender: number, id_receiver:  number, status?: number, created_by?: number, updated_by?: number}) {
        ...
        try {
            ...
            // 2 - Add to friendship
            const create_friendship_sql = 'insert into friendship(id_sender, id_receiver, created_by, updated_by, status) values($1, $2, $3, $4, $5) returning *;';
            const create_friendship_params = [id_sender, id_receiver, created_by, updated_by, status];
            const result_friendship = await client.query(create_friendship_sql, create_friendship_params);

            // 3 - Create chat room for users
            const create_chatRoom_sql = 'insert into _rooms(room_info_id, room_type) values($1, $2) returning *;';
            ...
            debug.db({
                ...
                friendship: result_friendship.rows,
