...
module.exports = (app: Express) => {
    app.get("/departments/get_list", filterGetList, async (req, res) => {
        ...
        debug.debugger(`/departments/get_list query`, JSON.stringify({size, page, search, sort, offset, limit}));

        try {
            const departments = await service.getAll({offset, limit, search, sort});
            const totalCount = await service.count({search});
            const totalPage = size > 0 ? Math.ceil(totalCount / size) : 1;
            debug.debugger(`/departments/get_list success`, JSON.stringify({size, page, search, sort, totalPage}));
    ...
    app.get("/departments/get_by_id/:id", async (req, res) => {
        ...
        try {
            ...
            debug.debugger(`/departments/${id} result`, JSON.stringify(user));
            if (!user) {
            ...
            debug.debugger(`/departments/${id} error`, e.message);
            return res.status(400).json({
                success: false,
                statusCode: 400,
    ...
    app.post("/departments/create", async (req, res) => {
        ...
        try {
            ...
            debug.debugger("/departments/create success", result);
            ...
            debug.debugger("/departments/create error", e.message);
            res.status(400).json({
                statusCode: 400,
                success: false,
                errorMessage: e.message,
    ...
    // app.put("/departments/updateById", async (req, res) => {
    ...
    //         debug.debugger("/departments/create success", result_id);
    //         return res.status(201).json({
    //             success: true,
    //             data: {
    //                 id: result_id
