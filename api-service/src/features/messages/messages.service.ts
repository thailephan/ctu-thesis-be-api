...
module.exports = {
    ...
    //     }
    // },
    async getNewMessages({roomId, from, to}) {
        const params = [];
        let sql = `
            select
                "senderId",
                ...
                "fullName" as "senderfullName",
                "avatarUrl" as "senderAvatarUrl"
            from messages
                join users on messages."senderId" = users.id
                join userAccounts uA on users.id = uA."userId"
            where messages.status = 1
        `;

        if (roomId) {
            params.push(roomId);
            sql += ` and "roomId" = $${params.length}`;
        } else {
            throw Error("Room id not found");
        }

        sql += ` and "messageId" > ${from || 1} `;
        if (to) {
            params.push(to);
            sql += ` and "messageId" < $${params.length};`;
        }
